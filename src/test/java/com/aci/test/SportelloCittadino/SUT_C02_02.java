...
public class SUT_C02_02 {
    ...
    @Test
    public void ACI_Sportello_Tasse() throws InterruptedException {
        Thread.sleep(4000);
        Sportello.setOpzioneTasse();
        Thread.sleep(4000);
        Sportello.setSelezioneRegione();
        Thread.sleep(2000);
        Sportello.clickEsenzione();
        Thread.sleep(2000);
        Sportello.clickEsenzioneStorico();
        Thread.sleep(2000);

        List<WebElement> selezioneUfficio = driver.findElements(By.xpath("//input[@class=\"widget-combobox-input\"]"));
        List<WebElement> selezioneOrario = driver.findElements(By.xpath("//input[@class=\"widget-combobox-input\"]"));

        if(selezioneUfficio.stream().noneMatch(WebElement::isDisplayed)
                && selezioneOrario.stream().noneMatch(WebElement::isDisplayed)) {

            System.out.println("L'appuntamento è online e correttamente non si vedono uffici o slot, prosegui pure");
        }
